...
func (c *UserClient) QueryAccount(u *User) *UserAccountQuery {
	...
	id := u.ID
	query.sql = sql.Select().From(sql.Table(useraccount.Table)).
		Where(sql.EQ(user.AccountColumn, id))
...
// QueryOwner queries the owner edge of a UserAccount.
func (c *UserAccountClient) QueryOwner(ua *UserAccount) *UserQuery {
	query := &UserQuery{config: c.config}
	id := ua.ID
	t1 := sql.Table(user.Table)
	t2 := sql.Select(useraccount.OwnerColumn).
		From(sql.Table(useraccount.OwnerTable)).
		Where(sql.EQ(useraccount.FieldID, id))
	query.sql = sql.Select().From(t1).Join(t2).On(t1.C(user.FieldID), t2.C(useraccount.OwnerColumn))
